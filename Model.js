...
function isPriorityChat(chat, priorityName) {
  ...
  if (normalizedName(chatTitle(chat)) === wanted) return true
  return !!chat.isGroup && !chat.lastFromMe && normalizedName(chat.lastSender) === wanted
}

function hasPriorityUnread(chats, priorityName) {
  var list = chats || []
  for (var i = 0; i < list.length; i++) {
    if (isPriorityChat(list[i], priorityName)) return true
  }
  return false
}
