...
Hover over the bar icon for an unread summary grouped by conversation. Click the summary or the bar icon to open the plugin's chat panel and reply. Use `j`/`k` or arrow keys to select a chat, `Enter` to open it, and `Escape` to go back. Opening a chat marks it read on WhatsApp. Right-click the bar icon for the full WhatsApp Web client.
