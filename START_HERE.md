...
Hover over the bar icon for an unread summary grouped by conversation. Click it to see the full chat list and reply. Use `j`/`k` or arrow keys to select a chat, `Enter` to open it, and `Escape` to go back. Opening a chat marks it read on WhatsApp.
