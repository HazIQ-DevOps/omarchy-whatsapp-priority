...
- Outgoing voice notes can be recorded from the composer with a microphone button or `Ctrl+Shift+R`, previewed, discarded, and explicitly sent as Ogg Opus push-to-talk audio. Recordings stay in private runtime files and stop after three minutes.
- Voice-note Settings list microphone sources so recording can use a specific working input without changing the system default. The choice is saved outside the plugin directory and survives shell restarts and plugin updates. Near-silent capture is rejected with an actionable error before sending.
...
- Both Groups headings show the total unread messages in visible groups, including muted groups. All three media players track the system's current default audio output so Bluetooth device changes take effect without restarting the shell.
- Video bubbles and gallery tiles use an embedded WhatsApp JPEG poster when present, or an FFmpeg frame from a video already in the local cache. Documents have type-specific icons, and outbound text links use Baileys' preview support when a site supplies metadata.
