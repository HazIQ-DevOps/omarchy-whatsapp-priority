...
Usage: ./install.sh [options]
  ...
  --replace      replace an existing WhatsApp plugin installation
...
if [[ $(readlink -f "$SOURCE_DIR") == $(readlink -f "$PLUGIN_DIR") ]]; then
  ...
  if [[ ( -e $PLUGIN_DIR || -L $PLUGIN_DIR ) && $replace_existing -eq 0 ]]; then
    die "$PLUGIN_DIR already exists. Review it first, then run ./install.sh --replace if you want to replace its plugin files."
  fi
